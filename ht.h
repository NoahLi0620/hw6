...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if(h == npos)
    {
        throw std::logic_error("HashTable insert: no free location found");
    }
    if(table_[h] == nullptr)
    {
...
{
    const size_t numCaps = sizeof(CAPACITIES)/sizeof(HASH_INDEX_T);
    if(mIndex_ + 1 >= numCaps)
    {
        throw std::logic_error("HashTable resize: no more capacities available");
    }
    ...
    for(HASH_INDEX_T i = 0; i < oldCap; ++i)
    {
        ...
        HASH_INDEX_T h = hash_(item->item.first) % newCap;
        Prober localProber = prober_;  
        localProber.init(h, newCap, item->item.first);
        ...
        if(loc == Prober::npos)
        {
            throw std::logic_error("HashTable resize: could not place rehashed item");
        }
    }
